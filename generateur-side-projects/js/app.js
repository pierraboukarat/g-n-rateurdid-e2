...
const dashboardFormSteps = [
  {
    ...
    fields: [
      ["comportement", "Quel comportement observez-vous chez elle ?"],
      ["probleme", "Quel problème rencontre-t-elle ?"],
      ["constat", "Quel constat vous fait penser qu'il y a une opportunité ?"],
    ],
    resetField: "get",
  ...
  },
  {
    ...
    fields: [
      ["obtenir", "Quel résultat veut-elle obtenir ?"],
      ["eviter", "Qu'est-ce qu'elle veut éviter ?"],
      ["desirabilite", "Pourquoi cette promesse est-elle désirable ?"],
    ],
    resetField: "to",
  ...
  },
  {
    ...
    fields: [
      ["premiereVersion", "Quelle première version pouvez-vous lancer ?"],
      ["testRapide", "Quel test rapide pouvez-vous faire ?"],
      ["signal", "Quel signal vous dira que ça intéresse vraiment quelqu'un ?"],
    ],
    resetField: "by",
        ...
        ${
          isLastStep
            ? `
              ...
              <button id="receiveDashboardButton" class="button receive-button" type="button" data-dashboard-action="lead">
...
function getDashboardPayload(email = "") {
  ...
  const getValue = dashboardState.cible.resume || liveGetResult?.value || normalizedIdea.get;
  const toValue = dashboardState.promesse.resume || liveToResult?.value || normalizedIdea.to;
  const byValue = dashboardState.action.resume || liveByResult?.value || normalizedIdea.by;

  return {
    email,
    source: "generateur-side-project-dashboard",
    type_demande: "recevoir_tableau_de_bord",
...
async function sendToN8n(payload) {
  console.log("Payload envoyé à n8n :", payload);

  try {
    await fetch(WEBHOOK_LEAD_URL, {
      method: "POST",
      mode: "no-cors",
      headers: {
...
async function submitLead(event) {
  ...
  console.log("✅ submitLead déclenché");
  console.log("WEBHOOK_LEAD_URL =", WEBHOOK_LEAD_URL);
  ...
  if (!leadEmail.checkValidity() || !trimmedEmail || !consentEmailStorage.checked) {
    leadMessageArea.innerHTML = notificationHtml(
      "Merci de renseigner votre email et d'accepter la conservation de votre adresse.",
      "is-danger"
    );
    return;
  }
  ...
  try {
    ...
    const payload = getDashboardPayload(trimmedEmail);
    console.log("payload =", payload);

    const result = await sendToN8n(payload);

    if (!result.success) {
      throw new Error("Erreur envoi n8n");
    }

    console.log("Requête envoyée à n8n");

    leadMessageArea.innerHTML = notificationHtml(
      "C'est envoyé. Votre tableau de bord arrive bientôt dans votre boîte mail.",
      "is-success"
    );
    leadForm.reset();
    closeLeadModal();
    ...
    console.error("[debug temporaire] Erreur submitLead :", error);
    leadMessageArea.innerHTML = notificationHtml(
...
const receiveDashboardButton = document.getElementById("receiveDashboardButton");

if (receiveDashboardButton) {
  receiveDashboardButton.addEventListener("click", openLeadModal);
}
